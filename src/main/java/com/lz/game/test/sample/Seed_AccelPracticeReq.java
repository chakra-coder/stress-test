 ...
 * Date: 13-5-28
 */
public class Seed_AccelPracticeReq extends AbstractSample
{
    @Override
    public void addParameter(Arguments params)
    {
        params.addArgument("player_id", "${player_id}");
        params.addArgument("yuanbao", "${yuanbao}");
    }
    @Override
    public MessageLite getRequest(JavaSamplerContext context)
    {
        MapProto.AccelPracticeReq.Builder req = MapProto.AccelPracticeReq.newBuilder();
        req.setPlayerId(context.getIntParameter("player_id"));
        req.setYuanbao(context.getIntParameter("yuanbao"));
        return req.build();
    }
    @Override
    public MessageLite doTest(MessageLite realReq) throws Exception
    {
        CommProto.RpcResponse resp = client.sendWaitBack(getRpcRequestBuilder(ServiceNameProto.ServiceName.MAP_VALUE, MapProto.MethodNameForMap.ACCEL_PRACTICE_VALUE, realReq));
        MapProto.AccelPracticeResp ret = MapProto.AccelPracticeResp.parseFrom(resp.getProtoParam());
        return ret;
    }
}
